...
class UpdateData():
    ...
    def UpdateTransactions(self):
        ...
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/465.l.43125/transactions'
    ...
    def UpdateLeague(self):
        # LEAGUE OVERVIEW
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/465.l.43125/'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    def UpdateLeagueStandings(self):
        # STANDINGS
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/league/465.l.43125/standings'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
    ...
    #My trial league printer

    def UpdateCurrentRosters(self):
        # WEEKLY ROSTERS - TAKES A WHILE
        yahoo_api._login()
        team = 1
        for team in range(1,13):
            url = 'https://fantasysports.yahooapis.com/fantasy/v2/team/465.l.43125.t.'+str(team)+'/roster/players'
            response = oauth.session.get(url, params={'format': 'json'})
            r = response.json()
            file_name = 'team_'+str(team)+ '_roster.json'
            with open('./rosters/Current/'+ file_name, 'w') as outfile:
                json.dump(r, outfile)
                print("Current roster for team " + str(team) +" - done")
            team =+ 1
        return;
        
    def LeagueLookup(self):
        yahoo_api._login()
        url = 'https://fantasysports.yahooapis.com/fantasy/v2/game/nhl'
        response = oauth.session.get(url, params={'format': 'json'})
        r = response.json()
        with open('GameLookup.json', 'w') as outfile:
            json.dump(r, outfile)
            return;
    ...
    def UpdateMatchups(self):
        ...
        for week in range(1,20):
            for team in range(1,current_week):
                url = 'https://fantasysports.yahooapis.com/fantasy/v2/team/465.l.43125.t.'+str(team)+'/matchups;weeks='+ str(week)
                response = oauth.session.get(url, params={'format': 'json'})
                r = response.json()
                file_name = 'team_'+str(team)+ '_matchup_week' + str(week) + '.json'
                with open('./matchups/'+ file_name, 'w') as outfile:
                    json.dump(r, outfile)
                    print("Current team for team " + str(team) + " week " + str(week) + " - done")
                team =+ 1
            week =+1
        return;


#ACKNOWLEDGE NOW THAT MAGIC DOES HAPPEN BELOW
def CurrentWeek():
...
class Bot():
    ...
    def run(self):
        ...
        UD.UpdateCurrentRosters()
        #UD.UpdateMatchups()
        print('Current rosters update - Done')
